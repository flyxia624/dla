		...
		ap_int<16> row_load_bound)
{
	...
	for(ap_uint<16> cycle=0;cycle<input_load_burst_length;cycle++)
	{
		...
		if(cycle>=8)
		{
			...
			else if( write_inwidth_counter.range(INBUFFER_WIDTH_BITWIDTH-1,0) == 0 )//取16次数，即8*16
			{
				buffer_address_mid++;
			}
			...
			if( write_inwidth_counter.range(2,0) == 0)
			{
				buffer_address_mid_offset = buffer_address_mid;//左边跳右边
			}
			else if(buffer_address_mini_tile == INDEPTH_MINITILE_SIZE-1)//取4层之后跳下一区域
			{
				buffer_address_mid_offset += inwidth_ceildiv_inbufferwidth;
			}
			...
			if(write_inwidth_counter == (ap_uint<9>) inwidth_align8)//取depth=8
			{
				write_inwidth_counter=1;
			}
			else
