...
)
{
	...
	for(ap_uint<16> outdepth_buffertile_idx=0;outdepth_buffertile_idx<weightbuffer_load_outdepth_number;outdepth_buffertile_idx++)//total mid number
	{

		buffer_address_mid_minitile_depth_offset = 0;

		for(int counter=0;counter<input_buffer_feeding_loop_bound;counter++ )//total number of tile 4*8 in row_step*out_width*indepth
		{
	...
	)
{
	...
	{		
		...
		ap_uint<128> temp128 = offseted_weight_DDR[address];
			...
			if(counter_x2/2==0)
			{
				for(int i=0;i<WINO_DOMAIN_SIZE_SQUARE/2;i++)//front 2 depth
				{
					#pragma HLS unroll
			...
			{
				for(int i=0;i<WINO_DOMAIN_SIZE_SQUARE/2;i++)//end two depth
				{
					#pragma HLS unroll
